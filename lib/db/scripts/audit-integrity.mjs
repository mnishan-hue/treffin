...
const baseChecks = [
  ["better_auth_identity_bridge_missing", `SELECT count(*)::int AS count FROM users WHERE better_auth_id IS NOT NULL AND clerk_id IS NULL`],
  ["orphan_comment_authors", `SELECT count(*)::int AS count FROM comments c LEFT JOIN users u ON u.id = c.author_id WHERE u.id IS NULL`],
  ...
  try {
    ...
    const compatibilityResult = await client.query(`
      WITH expected_columns(name) AS (
        VALUES ('article_id'), ('is_featured'), ('replies_locked'), ('sources'),
               ('word_count'), ('toxicity_flagged'), ('ai_suspected'), ('likes')
      ), expected_tables(name) AS (
        VALUES ('comment_likes'), ('comment_reactions')
      )
      SELECT
        (SELECT count(*)::int FROM expected_columns e WHERE NOT EXISTS (
          SELECT 1 FROM information_schema.columns c
          WHERE c.table_schema = current_schema() AND c.table_name = 'comments' AND c.column_name = e.name
        )) AS missing_columns,
        (SELECT count(*)::int FROM expected_tables e WHERE NOT EXISTS (
          SELECT 1 FROM information_schema.tables t
          WHERE t.table_schema = current_schema() AND t.table_name = e.name
        )) AS missing_tables
    `);
    const missingCompatibilityStructures =
      Number(compatibilityResult.rows[0]?.missing_columns ?? 0) +
      Number(compatibilityResult.rows[0]?.missing_tables ?? 0);
    if (missingCompatibilityStructures > 0) failed = true;
    process.stdout.write(`${missingCompatibilityStructures === 0 ? "PASS" : "FAIL"} comment_api_schema_compatibility: ${missingCompatibilityStructures}\n`);
