...
export default function ForgotPasswordPage() {
  ...
  return (
    <AuthShell>
      <h1 className="text-[1.65rem] font-bold text-foreground mb-1">Reset your password</h1>
      <p className="text-muted-foreground text-sm mb-8">We will email you a secure link if the account exists.</p>
      {sent ? (
        <div className="space-y-5" aria-live="polite">
          <div className="rounded-xl border border-emerald-500/25 bg-emerald-500/10 px-4 py-3 text-sm text-emerald-300">
            Check your inbox. For privacy, the same confirmation is shown for every email address.
          </div>
        ...
        <form onSubmit={submit} className="flex flex-col gap-4">
          <InputField label="Email" htmlFor="forgot-password-email">
            <input id="forgot-password-email" required type="email" autoComplete="email" value={email} onChange={(event) => setEmail(event.target.value)}
              placeholder="you@example.com" className="w-full rounded-xl px-4 py-3 text-sm text-foreground placeholder:text-muted-foreground/50 outline-none transition-all"
