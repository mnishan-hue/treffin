...
function ConfirmModal({
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-y-auto flex items-center justify-center bg-black/60 backdrop-blur-sm">
      <motion.div
...
function OutcomeModal({
  ...
  const [side, setSide] = useState<"support" | "against" | "draw">("support");
        ...
        <div className="space-y-4">
          <div>
            ...
            <div className="flex gap-2">
              {(["support", "against", "draw"] as const).map((s) => (
                <button
                  key={s}
                  onClick={() => setSide(s)}
...
function FreezeModal({
  ...
  async function submit() {
    ...
    try {
      ...
      setError(e instanceof Error ? e.message : "Failed to update freeze status");
    } finally {
      setSaving(false);
    }
  }
...
function Debates({ token }: { token: string }) {
  ...
  async function del(id: number) {
    try {
      ...
      setError(e instanceof Error ? e.message : "Failed to delete debate");
      setConfirm(null);
    }
  }
