...
import { X, Pen, Tag, FileText, CheckCircle, Eye, Image as ImageIcon, AlertCircle } from "lucide-react";
...
const MIN_WORDS = 500;
const DRAFT_KEY = "treffin:article-draft";
...
function saveDraft(draft: Omit<ArticleDraft, "savedAt">) {
  try {
    ...
    // storage unavailable (private browsing, quota) — draft saving is best-effort
  }
}
...
function WriteArticleModal({ onClose }: { onClose: () => void }) {
  ...
  const [peerReview, setPeerReview] = useState(initialDraft?.peerReview ?? false);
      ...
      },
      {
        ...
        onError: (err: unknown) => {
          ...
          const msg =
            serverMsg ??
            (status === 401
              ? "Your session may have expired. Try signing in again before publishing."
              : "Please try again.");
          setPublishError(msg);
        ...
        <div className="p-6 flex flex-col gap-5">
          {step === "write" ? (
            <>
              {restoredDraft && (
                <div
                  className="flex items-center gap-2 rounded-lg border border-primary/25 bg-primary/5 px-3 py-2 text-xs text-muted-foreground"
                  data-testid="text-draft-restored"
                >
                  <FileText className="w-3.5 h-3.5 text-primary shrink-0" />
                  <p className="flex-1">Restored your unsaved draft.</p>
                  <button
                    onClick={handleDiscardDraft}
                    className="font-semibold text-primary/80 hover:text-primary shrink-0"
              ...
              <input
                ...
                value={title}
                onChange={e => { setTitle(e.target.value); setPublishError(null); }}
