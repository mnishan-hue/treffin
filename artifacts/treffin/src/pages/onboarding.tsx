  ...
  return (
    <div className="flex flex-col items-center gap-3">
      <img
        ...
        alt="Treffin"
        className={cn("w-auto object-contain drop-shadow-[0_0_28px_rgba(139,92,246,0.8)]", iconH)}
      />
      ...
      <div className="relative z-10 flex flex-col items-center text-center px-6 max-w-md w-full py-16">
        ...
        >
          ...
          <div className="flex flex-col gap-1.5">
            <label className="text-[11px] font-semibold text-white/50 uppercase tracking-widest text-left">
            ...
            <div className="relative">
              ...
              <input
                ...
                onKeyDown={(e) => { if (e.key === "Enter" && name.trim()) onGetStarted(); }}
                className="w-full bg-white/7 border border-white/12 rounded-xl pl-10 pr-4 py-3 text-sm text-white placeholder:text-white/30 focus:outline-none focus:ring-2 focus:ring-indigo-500/50 focus:border-indigo-500/50 transition-all"
          ...
          <button
            className={cn(
              "w-full py-3.5 rounded-xl font-bold text-white text-sm transition-all active:scale-[0.98]",
              name.trim()
                ? "bg-gradient-to-r from-indigo-600 to-blue-600 hover:from-indigo-500 hover:to-blue-500 shadow-[0_0_24px_rgba(79,106,247,0.4)]"
  ...
  return (
    ...
    >
      <div className="bg-card border border-border/60 rounded-2xl p-7 shadow-2xl">
        ...
        <QuizNav onNext={onNext} nextDisabled={!name.trim() || !focus} onSkip={onSkip} />
      </div>
    </motion.div>
  );
}
...
export default function Onboarding() {
  ...
  const [name, setName] = useState(() => localStorage.getItem("treffin_name") ?? "");
  ...
  const finish = async () => {
    ...
    // Signal the welcome modal to appear after home loads
    localStorage.setItem("treffin_show_welcome", "1");
  ...
  const skipToEnd = () => {
    localStorage.setItem("treffin_onboarded", "true");
    if (name.trim()) localStorage.setItem("treffin_name", name.trim());
    setLocation("/");
  };
  ...
  if (phase === "welcome") {
    return (
      <div className="dark flex flex-col min-h-screen">
        <WelcomeScreen name={name} onChangeName={setName} onGetStarted={handleGetStarted} />
        <TrustFooter />
      </div>
    );
  }
