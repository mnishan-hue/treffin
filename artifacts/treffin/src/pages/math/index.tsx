...
import { MathWelcomeModal } from "@/components/math-welcome-modal";
...
/* ── Plain text preview (strips LaTeX for card snippets) ───────── */
...
/* ── Top Elegant Battles (most engaging showdowns) ─────────────── */
const MEDAL_STYLE = [
  { bg: "rgba(251,191,36,0.18)", border: "rgba(251,191,36,0.5)", color: "#fbbf24", medal: "🥇" },
  { bg: "rgba(148,163,184,0.18)", border: "rgba(148,163,184,0.45)", color: "#cbd5e1", medal: "🥈" },
...
function TopElegantBattles() {
  ...
  const mathDebates = (allDebates ?? []).filter(
    (d) => (d as typeof d & { mathProblemId?: number | null }).mathProblemId != null
      || d.title.startsWith("Elegance Battle:")
  );
  ...
  if (isLoading) {
    return (
      <div style={{ marginTop: 28 }}>
        <div style={{ height: 22, width: 240, borderRadius: 6, background: "var(--color-secondary)", marginBottom: 14 }} />
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(260px, 1fr))", gap: 12 }}>
          {[1, 2, 3].map(i => (
            <div key={i} style={{ height: 140, borderRadius: 16, background: "var(--color-secondary)" }} />
          ))}
        </div>
      </div>
    );
  ...
  if (mathDebates.length === 0) return null;

  return (
    <div
      style={{
        marginTop: 28, marginBottom: 8, position: "relative",
        borderRadius: 20, padding: "22px 22px 24px", overflow: "hidden",
        background: "linear-gradient(135deg, rgba(139,92,246,0.09) 0%, rgba(99,102,241,0.06) 55%, rgba(59,130,246,0.05) 100%)",
    ...
    >
      ...
      <div style={{ position: "absolute", bottom: -50, left: "30%", width: 180, height: 180, borderRadius: "50%", background: "radial-gradient(circle, rgba(96,165,250,0.12) 0%, transparent 70%)", pointerEvents: "none" }} />

      {/* Section header */}
      <div style={{ position: "relative", display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 18, flexWrap: "wrap", gap: 10 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <div style={{ width: 34, height: 34, borderRadius: 10, background: "linear-gradient(135deg,#8b5cf6,#6366f1)", border: "1px solid rgba(139,92,246,0.4)", display: "flex", alignItems: "center", justifyContent: "center", boxShadow: "0 4px 14px rgba(139,92,246,0.35)" }}>
            <Swords style={{ width: 17, height: 17, color: "white" }} />
          </div>
          <div>
            <div style={{ fontSize: "1.08rem", fontWeight: 900, color: "var(--color-foreground)", letterSpacing: "-0.01em" }}>
              ⚔ Elegant Battles
            </div>
        ...
        <Link href="/debates?category=Mathematics">
          <span style={{ display: "inline-flex", alignItems: "center", gap: 5, fontSize: "0.82rem", color: "#a78bfa", fontWeight: 700, cursor: "pointer" }}>
            See all debates →
          </span>
        </Link>
      </div>
      ...
      {/* Active battle cards */}
      <div style={{ position: "relative", display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(260px, 1fr))", gap: 12 }}>
        {topBattles.map((debate, i) => <BattleCard key={debate.id} debate={debate} rank={i} />)}
      </div>

      {/* Archived battles toggle */}
      {concludedBattles.length > 0 && liveBattles.length > 0 && (
        <div style={{ marginTop: 16, borderTop: "1px solid rgba(148,163,184,0.15)", paddingTop: 14 }}>
          <button
            onClick={() => setShowArchive(v => !v)}
            style={{
              display: "flex", alignItems: "center", gap: 6,
              fontSize: "0.78rem", fontWeight: 700, color: "var(--color-muted-foreground)",
              background: "none", border: "none", cursor: "pointer", padding: 0,
            }}
          >
            <span style={{ fontSize: "0.9em" }}>{showArchive ? "▾" : "▸"}</span>
            {concludedBattles.length} concluded battle{concludedBattles.length === 1 ? "" : "s"}
          </button>
          {showArchive && (
            <div style={{ marginTop: 10, display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(240px, 1fr))", gap: 10 }}>
              {concludedBattles.map((debate, i) => <BattleCard key={debate.id} debate={debate} rank={i} />)}
            </div>
          )}
        </div>
      )}
...
export default function MathHub() {
  ...
  const { data: allDebates }  = useGetDebates();
  ...
  // Map problem ID → its elegance battle (one per problem)
  const battlesMap = useMemo(() => {
    const map = new Map<number, { debateId: number; isLive: boolean }>();
    for (const d of allDebates ?? []) {
      const mpId = (d as typeof d & { mathProblemId?: number | null }).mathProblemId;
      if (mpId != null && !map.has(mpId)) {
        map.set(mpId, { debateId: d.id, isLive: !!d.isLive });
      }
    }
    return map;
  }, [allDebates]);
  ...
  return (
    <div style={{ margin: "-16px -16px 0", minHeight: "calc(100vh - 60px)" }}>
          ...
          <div className="math-hero-grid">
            ...
            <div>
              ...
              <div style={{ marginBottom: 24, display: "flex", alignItems: "center" }}>
                <SectionInfo title="Math Hub" icon="∑" accent="from-violet-500 to-fuchsia-600" description="Explore rigorous problems, write elegant proofs, challenge solutions in live debates, and build your mathematical reputation. All inside Treffin." />
              </div>
      ...
      {/* ── Top Elegant Battles (near contests, high engagement) ── */}
        ...
        <div>
          ...
          {probLoading ? (
            ...
            <div style={{
              ...
              borderRadius: 20, color: "var(--color-muted-foreground)",
            }}>
              <div style={{ fontSize: "2.5rem", marginBottom: 12, opacity: 0.5 }}>
                {debouncedSearch ? "🔍" : "∅"}
              </div>
              <p style={{ fontSize: "1rem", fontWeight: 700, color: "var(--color-foreground)", marginBottom: 6 }}>
                {debouncedSearch ? `No problems matching "${debouncedSearch}"` : "No problems here yet"}
              </p>
              <p style={{ fontSize: "0.82rem", marginBottom: 20, opacity: 0.7 }}>
                {debouncedSearch ? "Try a different search term or clear filters." : "Be the first to post a beautiful problem to this arena."}
              </p>
              {debouncedSearch ? (
                <button onClick={() => setSearch("")} style={{ padding: "8px 20px", borderRadius: 10, fontSize: "0.8rem", fontWeight: 600, background: "var(--color-secondary)", border: "1px solid var(--color-border)", color: "var(--color-muted-foreground)", cursor: "pointer" }}>
                  Clear search
                </button>
              ) : (
                <Link href="/math/post">
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
              {displayedProblems.map((p, i) => {
                const catColor = branchColor(p.categoryName ?? "");
                const preview = p.body ? plainPreview(p.body) : "";
                const battle = battlesMap.get(p.id);
                return (
                  <Link key={p.id} href={`/math/problem/${p.id}`} style={{ display: "block" }}>
                    ...
                    >
                      <div style={{ display: "flex", alignItems: "flex-start", gap: 14 }}>
                        {/* Rank circle */}
                        <div className="math-prob-rank" style={{
                        ...
                        <div style={{ flex: 1, minWidth: 0 }}>
                          ...
                          <div style={{ display: "flex", alignItems: "center", gap: 5, marginBottom: 5, flexWrap: "wrap" }}>
                            {(p.viewCount ?? 0) > 100 && (
                              <span style={{ fontSize: "0.63rem", background: "rgba(239,68,68,0.12)", color: "#f87171", borderRadius: 5, padding: "2px 7px", fontWeight: 800, letterSpacing: "0.04em" }}>🔥 HOT</span>
                            )}
                            {p.isProblemOfWeek && (
                              <span style={{ fontSize: "0.63rem", background: "rgba(251,191,36,0.1)", color: "#fbbf24", borderRadius: 5, padding: "2px 7px", fontWeight: 700 }}>⭐ POTW</span>
                            )}
                            {p.isFeatured && (
                              <span style={{ fontSize: "0.63rem", background: "rgba(99,102,241,0.1)", color: "hsl(231 89% 68%)", borderRadius: 5, padding: "2px 7px", fontWeight: 700 }}>✦ Featured</span>
                            )}
                            {battle && (
                              <span style={{
                                fontSize: "0.63rem",
                                background: battle.isLive ? "rgba(99,102,241,0.15)" : "rgba(148,163,184,0.1)",
                                color: battle.isLive ? "#a5b4fc" : "#94a3b8",
                                borderRadius: 5, padding: "2px 7px", fontWeight: 700,
                                border: `1px solid ${battle.isLive ? "rgba(99,102,241,0.3)" : "rgba(148,163,184,0.18)"}`,
                                display: "inline-flex", alignItems: "center", gap: 3,
                              }}>
                                {battle.isLive ? (
                                  <><span style={{ width: 4, height: 4, borderRadius: "50%", background: "#a5b4fc", display: "inline-block", animation: "liveBlink 1.2s ease-in-out infinite" }} /> Battle Live</>
                                ) : "⚔ Battle"}
                              </span>
                            )}
                          </div>
                          ...
                          <h3 style={{
                            ...
                            color: "var(--color-foreground)", margin: "0 0 5px",
                            display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical", overflow: "hidden",
                            lineHeight: 1.45,
                          }}>
                          ...
                          {preview && (
                            <p style={{
                              fontSize: "0.79rem", color: "var(--color-muted-foreground)",
                              margin: "0 0 8px",
                          ...
                          <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
                            {p.categoryName && (
                              <span style={{
                                fontSize: "0.68rem", background: "var(--color-secondary)",
                                border: `1px solid ${catColor}35`, borderRadius: 6, padding: "2px 8px",
                            ...
                            <span style={{ fontSize: "0.68rem", color: "var(--color-muted-foreground)", display: "flex", alignItems: "center", gap: 3 }}>
                              {(p.solutionCount ?? 0) > 0 ? (
                                <><span style={{ color: "#22c55e", fontWeight: 700 }}>✓</span> {p.solutionCount} solution{p.solutionCount !== 1 ? "s" : ""}</>
                              ) : (
                                <span style={{ opacity: 0.55 }}>No solutions</span>
                              )}
                            </span>
                            {(p.viewCount ?? 0) > 0 && (
                              <span style={{ fontSize: "0.68rem", color: "var(--color-muted-foreground)", opacity: 0.5 }}>{p.viewCount} views</span>
                            )}
                            {p.userName && (
                              <span style={{ fontSize: "0.68rem", color: "var(--color-muted-foreground)", display: "flex", alignItems: "center", gap: 4, marginLeft: "auto" }}>
                                <span style={{ width: 14, height: 14, borderRadius: "50%", background: `${catColor}25`, color: catColor, display: "inline-flex", alignItems: "center", justifyContent: "center", fontSize: "0.55rem", fontWeight: 900 }}>
                        ...
                        <div className="math-prob-actions" style={{ flexShrink: 0, display: "flex", alignItems: "center", gap: 8, paddingTop: 2 }}>
                          ...
                          <span style={{
                            ...
                            background: `${catColor}18`, border: `1px solid ${catColor}45`,
                            color: catColor, whiteSpace: "nowrap",
                          }}>
                            Solve →
                          </span>
                        </div>
