...
export default function MathProblems() {
  ...
  const changeStatus = async (id: number, status: string) => {
    try {
      await api.patch(`/admin/math-problems/${id}`, { status });
      setProblems((prev) => prev.map((p) => p.id === id ? { ...p, status } : p));
      if (selectedProblem?.id === id) setSelectedProblem((p) => p ? { ...p, status } : p);
      setMessage(`Problem #${id} status → ${status}`);
    } catch {
          ...
          {problems.map((p) => (
            ...
            >
              ...
              <div className="flex items-center gap-1 mt-2" onClick={(e) => e.stopPropagation()}>
                {(["open", "locked", "archived"] as const).filter((s) => s !== p.status).map((s) => (
                  <button
                    key={s}
                    onClick={() => void changeStatus(p.id, s)}
