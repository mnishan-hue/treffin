  ...
  const fetchUrgentCounts = async () => {
    try {
      const [stats, counts] = await Promise.all([
        api.get<{ flaggedPosts: number; pendingReviews: number; openAppeals: number }>("/admin/stats"),
        api.get<{ openAppeals: number; pendingReviews: number; openCreatorReports: number }>("/admin/notifications/counts").catch(() => ({ openAppeals: 0, pendingReviews: 0, openCreatorReports: 0 })),
      ]);
      setUrgentCounts({
        flaggedPosts: stats.flaggedPosts ?? 0,
        pendingReviews: counts.pendingReviews ?? stats.pendingReviews ?? 0,
        openAppeals: counts.openAppeals ?? stats.openAppeals ?? 0,
        openCreatorReports: counts.openCreatorReports ?? 0,
      });
    } catch {
