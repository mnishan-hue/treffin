...
function getBadge(id: Section, counts: UrgentCounts): string | null {
  ...
  if (id === "creator-reports" && counts.openCreatorReports > 0) return String(counts.openCreatorReports);
  return null;
}

export default function Sidebar({ active, onSelect, onLogout, onClose, urgentCounts }: SidebarProps) {
  const totalUrgent = urgentCounts
    ? urgentCounts.flaggedPosts + urgentCounts.pendingReviews + urgentCounts.openAppeals + urgentCounts.openCreatorReports
    : 0;
